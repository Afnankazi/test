...
public class Main {
    ...
    public static void updateacc() throws IOException{
      Writer w =new  BufferedWriter(new BufferedWriter(new FileWriter("data/try.txt")));
      
      Files.lines(paths[0]) 
      .forEach(line -> {
          String[] words = line.split(" ");
          if(words[0].equals("CASH")){
          words[2] =cashAccountService.retrieveTradeAccount(words[1]).getCashBalance().toString();
          }else{
            marginAccountService.retrieveTradeAccount(words[1]).getMargin().toString();
